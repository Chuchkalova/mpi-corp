...
class UploadHandler
{
    ...
    function get_config_bytes($val) {
        $val = trim($val);
        $last = isset($val[strlen($val)-1])?strtolower($val[strlen($val)-1]):0;
        switch($last) {
            case 'g':
                $val *= intval($val)*1024;
            case 'm':
    ...
    protected function get_unique_filename($file_path, $name, $size, $type, $error,
		...
		$bytes_count=isset($content_range[1])?(int)$content_range[1]:0;
        $uploaded_bytes = $this->fix_integer_overflow($bytes_count);
    ...
    public function generate_response($content, $print_response = true) {
        ...
        if ($print_response) {
            ...
            $redirect = !empty($this->get_query_param('redirect'))?stripslashes($this->get_query_param('redirect')):"";
            if ($redirect) {
