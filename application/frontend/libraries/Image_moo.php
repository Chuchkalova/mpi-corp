...
class Image_moo
{
	...
	//----------------------------------------------------------------------------------------------------------
	{
		if (!is_resource($this->main_image)&&!($this->main_image instanceof \GdImage))
		{
	...
	//----------------------------------------------------------------------------------------------------------
	{
		if(is_resource($this->watermark_image)|| $this->watermark_image instanceof \GdImage) imagedestroy($this->watermark_image);
		$this->watermark_image = $this->_load_image($filename);

		if(is_resource($this->watermark_image)|| $this->watermark_image instanceof \GdImage)
		{
	...
	//----------------------------------------------------------------------------------------------------------
	{
		if (!is_resource($this->temp_image)&&!($this->temp_image instanceof \GdImage))
		{
	...
	//----------------------------------------------------------------------------------------------------------
	{
		if(is_resource($this->main_image)|| $this->main_image instanceof \GdImage) imagedestroy($this->main_image);
		if(is_resource($this->watermark_image)|| $this->watermark_image instanceof \GdImage) imagedestroy($this->watermark_image);
		if(is_resource($this->temp_image)|| $this->temp_image instanceof \GdImage) imagedestroy($this->temp_image);
		return $this;
	}
	...
	//----------------------------------------------------------------------------------------------------------
	{
		...
		// copy section
		imagecopyresampled($this->temp_image, $this->main_image, 0, 0, (int)$sx, (int)$sy, (int)$mw, (int)$mh, (int)$sx2, (int)$sy2);
		return $this;
	}
...
//		    if( $this->width > $this->height ) { could calc wronf - Cole Thorsen swapped to his suggestion
	...
	//----------------------------------------------------------------------------------------------------------
	{
		...
		// check it
		if(!is_resource($bu_image)&&!($bu_image instanceof \GdImage))
		{
